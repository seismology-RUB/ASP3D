...
class regions(object):
    ...
    def plotTracesInActiveRegions(self, event=None, keys='all', maxfigures=20, qt=False, qtMainwindow=None):
        '''
                        ...
                        if shot.getShotnumber() == shotnumber:
                            for traceID in self.shots_found[key]['shots'][shotnumber]:
                                ...
                                if ntraces > maxfigures:
                                    print('Maximum number of figures ({}) reached. Figure #{} for shot #{} '
                                          'will not be opened.'.format(maxfigures, ntraces, shot.getShotnumber()))
                                    break
        ...
        if ntraces > maxfigures and qt:
            reply = QtGui.QMessageBox.question(qtMainwindow, 'Message', 'Maximum number of opened figures is '
        ...
        for shot, traceID in traces2plot:
            if qt:
                repickingQt.append(Repicking_window(qtMainwindow, shot, traceID))
            else:
    ...
    def markRectangle(self, xtup, ytup, key=None, color='grey', alpha=0.1, linewidth=1):
        '''
